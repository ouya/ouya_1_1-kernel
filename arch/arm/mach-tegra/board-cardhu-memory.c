...
static const struct tegra_emc_table cardhu_emc_tables_hynix[] = {
	{
		0x32,       /* Rev 3.2 */
		25500,      /* SDRAM frequency */
		{
...
int cardhu_emc_init(void)
{
	...
	strap_pin_DDR = gpio_get_value(TEGRA_GPIO_PG4);
	printk("DDR Strap Pin AD4: %d\n", strap_pin_DDR);
	gpio_free(TEGRA_GPIO_PG4);

	if (strap_pin_DDR == 1) {
		printk("Init DFS table for Hynix DDR\n");
		tegra_init_emc(cardhu_emc_tables_hynix, ARRAY_SIZE(cardhu_emc_tables_hynix));
	} else {
